...
- The average overall rating I gave, out of 10, across all episodes and shows was `r mean(latest_data$overall_rating, na.rm = TRUE)`. This was `r mean(latest_data$overall_rating, na.rm = TRUE)-mean(latest_data$IMDB_rating, na.rm = TRUE)` points higher than the average critics' score of `r mean(latest_data$IMDB_rating, na.rm = TRUE)`.
...
rating.df <- melt(setDT(df1), id.vars='sitcom')
...
ggplot(rating.df, aes(x=sitcom, y = value, fill = variable)) +
  ...
  labs(title = "Which Sitcom is Best?",
       ...
       fill = "Rating Source") +
  scale_fill_manual(labels = c("Myself", "Critics (IMDB Score)"), values = c("#ffc8dd", "#cdb4db")) +
  theme_light()
```
...
FunninessEmotionality.df <- melt(setDT(df2), id.vars='sitcom')
...
ggplot(FunninessEmotionality.df, aes(x=sitcom, y = value, fill = variable)) +
  ...
  labs(title = "Are Sitcoms Funnier than they are Emotional?",
       ...
       y = "Average Score",
       fill = "Factor") +
  scale_fill_manual(labels = c("Funniness", "Emotional Impact"), values = c("#ffc8dd", "#cdb4db")) +
  theme_light()
```
